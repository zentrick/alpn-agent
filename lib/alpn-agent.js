...
const _rejectUnauthorized = Symbol('rejectUnauthorized')
...
class ALPNAgent extends Agent {
  ...
  async negotiateALPN (options) {
    ...
    if (options.ALPNProtocols.includes(ALPN_H2) && this[_h2].has(name)) {
      DEBUG('host known to negotiate h2')
      return ALPN_H2
    } else if (options.ALPNProtocols.includes(ALPN_H1) && this[_h1].has(name)) {
  ...
  _prepareOptions (options, protocols) {
    ...
    options.servername = options.servername || getHostName(options)
    options.ALPNProtocols = options.ALPNProtocols || protocols
    options.port = options.port || 443
    options.lookup = options.lookup || this[_lookup]
