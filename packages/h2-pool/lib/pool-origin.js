...
class Origin {
  ...
  get sessions () {
    return this[_active].size + this[_idle].size
  }
  ...
  _findActive () {
    ...
    for (const session of this[_active]) {
      if (hasAvailableStreams(session)) {
  ...
  _popIdle () {
    ...
    if (lastSession != null) {
      this[_idle].delete(lastSession)
      this[_pool]._popIdle(lastSession)
      this[_active].add(lastSession)
      this[_pool].reuseSession(lastSession)
